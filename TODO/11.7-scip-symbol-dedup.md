...
Tasks:
- [x] In `run_enrich()`, query existing tree-sitter symbols per file and
  build a `name + line → SymbolId` mapping (not qualified_name — see above)
- [x] Stop inserting SCIP symbol definitions (remove `insert_scip_symbol`
  calls, or gate them behind "unmatched only")
- [x] Two-pass: first build global `scip_symbol_string → tree_sitter_SymbolId`
  map across all documents, then insert remapped references in second pass
- [x] Remap SCIP reference targets to tree-sitter symbol IDs via the mapping
- [x] Update enrichment stats output (symbols_added → symbols_matched)
- [x] Add test: enrichment does NOT increase dead symbol count
