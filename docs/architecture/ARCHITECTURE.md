...
**SCIP enrichment** (implemented, optional precision upgrade):
- `statik enrich <scip-file>...` imports SCIP (Source Code Intelligence Protocol) index files into the existing SQLite database, adding compiler-resolved symbols and references alongside tree-sitter data. SCIP indexes are generated by external tools (`scip-typescript`, `scip-java`, `rust-analyzer`, `scip-clang`) and provide fully type-resolved references. SCIP-enriched files get upgraded confidence (Certain) in dead code and impact analysis. Staleness tracking ensures files edited after SCIP generation fall back to tree-sitter. See `src/scip/mod.rs` for the reader implementation.
...
- ~~SCIP index consumption as optional enrichment~~ (delivered: `statik enrich` command)
- IDE extensions
