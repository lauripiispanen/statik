...
This is an optional enrichment step. Tree-sitter indexing (`statik index`) is always the fast path and works without a build. SCIP enrichment adds compiler-grade precision: for enriched files, SCIP-derived edges replace tree-sitter heuristic edges, eliminating false positives. All graph-based analyses benefit (`dead-code`, `impact`, `cycles`, `deps`, `lint`, `who`, `bus-factor`).
...
- For SCIP-enriched files, outgoing tree-sitter edges are replaced with SCIP-derived edges, eliminating false positives from unused imports and wrong wildcard resolution. Non-enriched files keep their tree-sitter edges unchanged. SCIP-derived edges appear with a `[scip]` marker in text output and `"is_scip_derived": true` in JSON output
...
- `statik summary` shows enrichment status (files enriched, stale files, tree-sitter-only files) and a resolution breakdown ("N files SCIP-precise, M tree-sitter-heuristic")
- All graph-based commands benefit from SCIP edges: `dead-code` (fewer false positives), `impact` (more complete blast radius), `cycles` (discovers hidden cycles), `deps` (shows call-based dependencies alongside import-based), `lint` (catches SCIP-discovered violations), and `who`/`bus-factor` (more accurate impact radius). Confidence is upgraded to Certain for SCIP-enriched files
