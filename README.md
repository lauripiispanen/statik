...
This is an optional enrichment step. Tree-sitter indexing (`statik index`) is always the fast path and works without a build. SCIP enrichment adds compiler-grade precision: cross-file call references from SCIP are added as supplemental edges in the file graph, improving all graph-based analyses (`dead-code`, `impact`, `cycles`, `deps`, `lint`, `who`, `bus-factor`).
...
- Cross-file call references from SCIP are added as supplemental edges in the file graph. These edges supplement import edges (they never replace them) and are deduplicated against existing import-based edges. SCIP-derived edges appear with a `[scip]` marker in text output and `"is_scip_derived": true` in JSON output
